...
<div class="flex flex-wrap lg:ml-12 md:ml-0.5 my-4 flex-col">
    ...
    <div class="flex flex-row w-full">
                ...
                <div
                    ...
                    <a class="flex h-full flex-col" href="{{ route('produit.details', [$produit->nom_prod]) }}">
                        <div class="my-auto ">
                            <img src="{{ Storage::url($produit->url_prod) }}" alt="Image du produit" class="w-fit h-full" />
                        </div>
                        <div class="mt-auto">
                            <h1 class="ml-2 font-extrabold text-center md:text-xs lg:text-xl my-1">{{ $produit->nom_prod }}
                            </h1>
                            <p class="text-center mt-auto"> {{ $produit->prix_prod }} Fcfa</p>
                        </div>
                    </a>
                    <button type="button "
                    ...
                    @else
                        <button  wire:click="addPanier({{ $produit->id }})" class="cursor-pointer dark:text-gray-700 flex mt-0 text-center py-2 px-5 hover:opacity-70 text-white rounded-lg mb-1 bg-green-600"
