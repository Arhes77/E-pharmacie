<div x-data="{isEnable: true}" class="relative">
    ...
    >
      ...
      <button class="  absolute top-0 right-0 h-full  ml-5 inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent
        ...
        <input 
            @click.away = "isEnable= false ; @this.resetIndex() ;" @click="isEnable = true"
            class="rounded-md w-full shadow-sm border-gray-300" type="text" 
            placeholder="Paracethamol, Tramadhol, Dollipranne..." autocomplete="off"
            wire:model="query"
            wire:keydown.arrow-down.prevent="incrementIndex"
            wire:keydown.arrow-up.prevent="decrementIndex"
            wire:keydown.backspace="resetIndex"
    ...
    <div class="absolute bg-gray-100 px-2 my-3 py-5 shadow-lg rounded w-full"
    x-show="isEnable">
        @if(count($produits)>0)
            ...
            @foreach ($produits as $index => $prod)
                <div class="flex items-center px-0 ">
                    ...
                    <img src="{{ Storage::url($prod->url_prod)}}" alt="...." class="w-10 h-10 mx-2 rounded-full">
